#!/usr/bin/env bash
# Frontend Docker image upload script for AWS SaaS Boost
...
ECR_TAG=${ECR_TAG:-"frontend_tag"}

if [ -z "$ECR_REPO" ]; then
    echo "Error: Could not retrieve ECR repository for frontend. Exiting."
    exit 1
fi
...
echo "Frontend image uploaded to $DOCKER_TAG"
